...
const App = () => {
  ...
  // Lógica de Saldos (Total vs Disponível 48h)
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-900 p-6">
        <form onSubmit={(e) => { e.preventDefault(); signInWithEmailAndPassword(auth, email, password); }} className="bg-white p-10 rounded-[2rem] w-full max-w-md shadow-2xl">
          ...
          <div className="space-y-4">
            <input type="email" placeholder="Email / NIF" className="w-full p-4 bg-slate-100 rounded-xl outline-none" onChange={e => setEmail(e.target.value)} required />
        ...
        {role === 'admin' && (
          <div className="grid gap-8">
            <div className="bg-slate-900 text-white p-10 rounded-[2.5rem] shadow-xl">
              <p className="text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Controlo Central</p>
              <h2 className="text-4xl font-black italic">Monitorização de Rede</h2>
            </div>
            <div className="bg-white p-6 rounded-2xl border">
               <p className="font-bold text-slate-400 text-xs uppercase mb-4">Ações Rápidas</p>
               <div className="flex gap-4">
                  <button className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold text-xs uppercase shadow-lg shadow-blue-100">Criar Nova Loja</button>
                  <button className="bg-slate-100 text-slate-600 px-6 py-3 rounded-xl font-bold text-xs uppercase">Ver Todos Clientes</button>
               </div>
            </div>
          </div>
        )}
        ...
        {role === 'comerciante' && (
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="bg-white p-10 rounded-[2.5rem] border shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input type="number" placeholder="Valor €" className="w-full p-4 bg-slate-50 rounded-xl text-2xl font-black outline-none focus:ring-2 focus:ring-blue-600" value={op.valor || ''} onChange={e => setOp({...op, valor: Number(e.target.value)})} />
            ...
            <div className="bg-white rounded-[2.5rem] border p-10">
               <h3 className="font-black text-[10px] uppercase text-slate-400 mb-6 tracking-widest">Últimos Movimentos</h3>
               <div className="space-y-3">
                  {movimentos.map(m => (
                    <div key={m.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                      <div><p className="text-[10px] font-black">{m.clienteId}</p><p className="text-[8px] text-slate-400">{m.docOrigem || 'Sem Doc'}</p></div>
        ...
        {role === 'cliente' && (
          <div className="max-w-md mx-auto">
            <div className="bg-slate-950 p-12 rounded-[3.5rem] text-white shadow-2xl flex flex-col items-center border-b-[12px] border-blue-600">
               <div className="bg-white p-6 rounded-[2.5rem] mb-10 shadow-2xl"><QRCodeSVG value={user.email || ""} size={160} /></div>
               ...
               <div className="w-full pt-8 border-t border-white/10 flex justify-between items-center">
                  <div className="text-left"><p className="text-[8px] font-black text-slate-500 uppercase">Total Teórico</p><p className="font-black text-lg">{total.toFixed(2)}€</p></div>
